...
void GRID::Turbulence( PROJECT* project )
{
  ...
  //     The turbulent kinetic energy "K" and the eddy viscosity "vt" are computed as
  //     the sum of the algebraic shear stress model and the Large-Eddy model
  ...
  else if( isFS(turb, BCONSET::kVtLES)  &&  isFS(turb, BCONSET::kVtAlgebraic) )
  {
    ...
    for( int n=0; n<np; n++ )
    {
      if( cnt[n] )
      {
        ...
        double Utau2 = node[n].cf * (U*U + V*V);
        ...
        double vt = ste[n] * H * Utau  +  L2 * sqrt( phi[n] );
        double K  = (L2*phi[n]  +  Utau2*sqrt(cK*ste[n])) / sqrt(cm*cd);
        // ... K  = (vt*vt/L/L/sqrt(cm*cd)  +  Utau*Utau*sqrt(cK*ste[n]/cm/cd);

        // -----------------------------------------------------------------------------------------
        // The following code is not consistent to the Large-Eddy model, because in the case
        // ste[n] = 0 the computation of NODE::uu = NODE::vv = -2/3*K is different to the
        // computed values in the LES model.
